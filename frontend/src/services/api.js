...
api.interceptors.response.use(
    (response) => {
        return response; // daca e ok 200 returneaza datele normal
    },
    (error) => {
...
export const ParticipantService = {
    ...
    create: (participant) => api.post('/participanti', participant),
    update: (id, updatedParticipant) => api.put(`/participanti/${id}`, updatedParticipant),
    delete: (id) => api.delete(`/participanti/${id}`)
};

// servicii prezenta participant
export const ParticipantPrezentaService = {
    getAll: () => api.get('/prezenta/lista'),
    getById: (id) => api.get(`/prezenta/${id}`),
    create: (prezenta) => api.post('/prezenta/creare', prezenta),
    update: (id, updatedAttendance) => api.put(`/prezenta/actualizare/${id}`, updatedAttendance),
    delete: (id) => api.delete(`/prezenta/stergere/${id}`)
};
...
export const TraseuService = {
    ...
    delete: (id) => api.delete(`/trasee/${id}`)
};
...
export const UserService = {
    ...
    getCoordonatori: () => api.get('/utilizatori/coordonatori/lista')
};
