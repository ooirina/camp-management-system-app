...
public interface ParticipantRepository extends  JpaRepository<Participant, Long>{
    ...
    // Filtrat pe CONFIRMAT + PLATIT, ca să corespundă cu raportul de bucătărie (porții reale necesare)
    @Query("SELECT DISTINCT p FROM Participant p " +
            ...
            "AND i.statut = 'CONFIRMAT' AND i.statusPlata = 'PLATIT' " +
    ...
    // Filtrat pe CONFIRMAT + PLATIT, ca să corespundă cu raportul de bucătărie
    @Query("SELECT DISTINCT p FROM Participant p " +
            "JOIN Inscriere i ON i.participant = p " +
    ...
    // Toți participanții unei tabere, indiferent de status (pentru lista generală a coordonatorului)
    @Query("SELECT DISTINCT p FROM Participant p JOIN Inscriere i ON i.participant = p WHERE i.tabara.id = :idTabara AND i.statut <> 'ANULAT' ORDER BY p.nume ASC")
    List<Participant> findByTabaraId(@Param("idTabara") Long idTabara);
}